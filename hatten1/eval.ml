...
let string_of_env env =
  Printf.sprintf "{%s}"
    ...
    |> List.filter_map (fun (x, v) ->
           ...
           | Some v -> Some (Printf.sprintf "%s=%s" x (string_of_value v)))
    |> String.concat ", ")
...
and eval_expr (env : dval option ref Env.t) (e : expr) : value =
  match e with
  | `EConstInt i -> VInt i
  ...
  | `EVar x -> (
      try
        ...
        | {contents=Some dval} as r -> let v = eval_dval dval in r := Some (DVal v); v
  ...
  | `ECall (e1, e2) -> (
      ...
      match v1 with
      | VFunc (x, e', oenv) ->
          let r = DThunk (e2, env) |> Option.some |> ref in
          eval_expr (oenv |> Env.extend x r) e'
      | VBuiltinFun (_, f) -> f (DThunk (e2, env))
  ...
  | `EFun (_, var, body) -> VFunc (var, body, env)
  | `ELet ((vars, e1s), e2) ->
      (* let v1s = List.map (eval_expr env) e1s in *)
      let env' =
        List.fold_left2
          (fun env0 var e ->
            let dthunk = DThunk (e, env) in
            env0 |> Env.extend var (ref @@ Option.some dthunk))
          env vars e1s
      in
      eval_expr env' e2
  ...
  | `EIf (e1, e2, e3) -> (
      ...
      | _ -> raise (TypeError (Value.string_of_value v1)))
...
let eval_command env (c : 'a command) =
  ...
  | CExp e ->
      if Options.inference_only then (
        ...
        let v = eval_dval (DThunk (e, env)) in
  ...
  | CDecls (names, es) ->
      if Options.inference_only then (
        ...
        let env' =
          env
          |> Env.extends names
               (List.map (fun e -> ref (Some (DThunk (e, env)))) es)
        in
...
let rec equal : value * value -> _ = function
  ...
  | VTuple xs, VTuple ys | VList xs, VList ys ->
      VBool
        ...
        && List.for_all2
             (fun x y -> equal (eval_dval x, eval_dval y) = VBool true)
             xs ys)
  | VCons (x, xs), VCons (y, ys) ->
      VBool
        (equal (eval_dval x, eval_dval y) = VBool true
        && equal (eval_dval xs, eval_dval ys) = VBool true)
  | VCons _, VList [] | VList [], VCons _ -> VBool false
  | VCons (x, xs), VList ys | VList ys, VCons (x, xs) ->
      VBool
        (equal (eval_dval x, eval_dval (List.hd ys)) = VBool true
        && equal (eval_dval xs, VList (List.tl ys)) = VBool true)
  | x, y -> raise (TypeError (string_of_value x ^ " = " ^ string_of_value y))

let builtins =
  let open Value in
  let wrapper (op : string) f =
    (op, VBuiltinFun (op, fun thunk -> f (eval_dval thunk)))
  in
  let wrapper2 op f =
    ( op,
      VBuiltinFun
        ( op,
          fun x ->
            VBuiltinFun (op ^ "'", fun y -> f (eval_dval x, eval_dval y)) )
    )
  in
  let wrapper2_lazy op f =
    (op, VBuiltinFun (op, fun x -> VBuiltinFun (op ^ "'", fun y -> f (x, y))))
  in
  let sv = string_of_value and sd = string_of_dval in
  [
    wrapper2 "=" equal;
    wrapper2 "+" (function
    ...
    wrapper2 "<=" (function
      | VInt x', VInt y' -> VBool (x' <= y')
      | x, y -> raise (TypeError (sv x ^ " <= " ^ sv y)));
    wrapper2_lazy "&&" (fun (dval1, dval2) ->
        match eval_dval dval1 with
        | VBool true -> eval_dval dval2
        | VBool false -> VBool false
        | x -> raise (TypeError (sv x ^ " && " ^ sd dval2)));
    wrapper2_lazy "||" (fun (dval1, dval2) ->
        match eval_dval dval1 with
        | VBool true -> VBool true
        | VBool false -> eval_dval dval2
        | x -> raise (TypeError (sv x ^ " || " ^ sd dval2)));
    wrapper "fst" (function
      | VTuple [ x'; _ ] -> eval_dval x'
      | x -> raise (TypeError ("fst " ^ sv x)));
    wrapper "snd" (function
      | VTuple [ _; y' ] -> eval_dval y'
      | x -> raise (TypeError ("snd " ^ sv x)));
    wrapper2_lazy "::" (function x, y -> VCons (x, y));
    wrapper "hd" (function
      | VCons (x', _) -> eval_dval x'
      | VList (x' :: _) -> eval_dval x'
      | VList [] -> raise (Invalid_argument "hd []")
      | x -> raise (TypeError ("hd " ^ sv x)));
    wrapper "tl" (function
      | VCons (_, xs) -> eval_dval xs
      | VList (_ :: xs) -> VList xs
...
let initial_env =
  ...
  empty |> extends ks (List.map (fun v -> ref @@ Option.some @@ DVal v) vs)
